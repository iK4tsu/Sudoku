...
public abstract class Sudoku
{
    ...
    this(in uint[][] puzzle)
    {
        grid = new Grid(9, 9, puzzle);
    }

    this(in uint x, in uint y, in uint[][] puzzle)
    {
        grid = new Grid(x, y, puzzle);
    }
    ...
    private uint backtrack(in uint column, in uint row)
    {
        ...
        {
            if (column < grid.dim.x - 1)   return backtrack(column + 1, row);
            else if (row < grid.dim.x - 1) return backtrack(0, row + 1);
            else                           return grid[row, column].n;
        }

        uint ret;
        for (uint i = 1; i <= grid.dim.x; i++)
        {
            if (!grid.find(row, column, i))
            {
                grid[row, column].n = i;

                if (column < grid.dim.x - 1)   ret = backtrack(column + 1, row);
                else if (row < grid.dim.x - 1) ret = backtrack(0, row + 1);
                else                           return i;
